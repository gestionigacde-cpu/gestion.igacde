...
Turnos, Clases, Alumnos, Recetas, la planificación semanal de grupos/recetas
por clase y la Lista de Compras semanal.
...
- `firebase-config.js` — credenciales del proyecto Firebase (a completar).
...
2. Te va a mostrar un objeto `firebaseConfig`. Copialo entero.
3. Abrí `firebase-config.js` en este proyecto y reemplazá el objeto de ejemplo por el que copiaste.
...
3. Cargá al menos una Carrera, un Turno, un Docente, un Ingrediente y una Receta para poder probar el flujo completo: Clases → Planificación semanal (grupos + receta) → Lista de compras.

## Flujo funcional (resumen)

1. Admin carga catálogos: Carreras, Turnos, Docentes, Alumnos, Ingredientes, Recetas (con sus ingredientes y cantidades).
2. Admin define las Clases (Carrera + Turno + Día + Docente, recurrente semana a semana).
3. Admin inscribe Alumnos en Carrera + Turno (un alumno puede tener varias inscripciones).
...
5. Admin entra a "Lista de compras", elige la semana, y genera la lista: el sistema suma los ingredientes de todas las recetas planificadas esa semana en todas las clases, en una única lista.
...
- Reportes/estadísticas (alumnos por carrera/turno, asistencia agregada, etc.).
